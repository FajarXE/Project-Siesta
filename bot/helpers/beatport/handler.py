...
# Pastikan mengimpor USER_AGENT dari api
...
async def refresh_track_url(item_id: str, current_meta: dict, user_id: int):
    """
    Mengambil URL baru (Just-in-Time) untuk menghindari link expired atau kosong.
    """
    try:
        # Ambil preferensi kualitas
        ...
        for qual in quality_priority:
            try:
                # Mapping kualitas string ke format API
...
async def start_beatport(url: str, user: dict):
    try:
        ...
        if media_type == 'artist':
            raise NotImplementedError("Unduhan Artis Beatport belum didukung sepenuhnya.")
...
async def download_beatport_track(url: str, filepath: str, proxy: str = None):
    try:
        # Gunakan USER_AGENT yang konsisten dari API
        headers = {
        ...
        timeout = aiohttp.ClientTimeout(total=1800) # 30 menit max
...
async def start_track(item_id: str, user: dict, track_meta: dict | None, upload=True, filepath=None, disable_link=False):
    async with BEATPORT_SEMAPHORE:
        ...
        # --- LOGIKA PENTING: JUST-IN-TIME URL ---
        # Jika URL kosong (karena mode album), ambil sekarang
        if not track_meta.get('download_url'):
            ...
            if new_url:
                ...
                LOGGER.error(f"Skip track {item_id}: Gagal mendapatkan URL.")
                return False
        ...
        # Retry jika 403 (URL Expired)
        if err == "403_FORBIDDEN":
            LOGGER.warning(f"URL 403. Refreshing {item_id}...")
            new_url, qual = await refresh_track_url(item_id, track_meta, user_id)
            if new_url:
                track_meta['download_url'] = new_url
                if qual and qual != track_meta['quality'].lower():
                     # Update path jika ekstensi berubah
        ...
        if err:
            LOGGER.error(f"Beatport Fail {item_id}: {err}")
            return False
