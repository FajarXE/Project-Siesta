# [FILE BARU: bot/helpers/beatport/metadata.py]
...
# Kita akan menyederhanakannya. Kita akan selalu mencoba lossless.
# API akan menangani jika 'lossless' tidak tersedia berdasarkan langganan.
QUALITY_MAP = {
...
async def process_track_metadata(track_id: str, r_id: str, user: dict, pre_data: dict = None):
    ...
    # Kita coba lossless dulu, lalu fallback
    try:
        stream_data = await client.get_track_download(track_id, QUALITY_MAP['lossless'])
        metadata['quality'] = "FLAC"
        metadata['extension'] = "flac" # Asumsi lossless adalah FLAC
    except Exception:
        try:
            stream_data = await client.get_track_download(track_id, QUALITY_MAP['high'])
            metadata['quality'] = "AAC 256"
            metadata['extension'] = "m4a" # AAC biasanya .m4a
        except Exception:
            stream_data = await client.get_track_download(track_id, QUALITY_MAP['medium'])
            metadata['quality'] = "AAC 128"
            metadata['extension'] = "m4a"
            
    metadata['download_url'] = stream_data.get("location")
    if not metadata['download_url']:
        raise BeatportError("Gagal mendapatkan URL download (Mungkin masalah langganan).")
