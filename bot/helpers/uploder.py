...
def create_cloud_caption(metadata):
    ...
    # Tampilan simpel untuk Playlist
    ...
    # Tampilan standar untuk Album/Artist
...
async def upload_to_cloud_handler(filepath, user, metadata, mode):
    ...
    try:
        ...
        # ==========================================
        # KASUS 2: FOLDER (BELUM ZIP)
        # ==========================================
        elif os.path.isdir(filepath):
            
            # --- GOFILE: Manual Folder Creation ---
            if mode == 'Gofile':
            ...
            # --- BUZZHEAVIER: Native Folder Support ---
            elif mode == 'Buzzheavier':
            ...
            # --- VIKINGFILES: Auto Zip (No Folder Support) ---
            elif mode == 'Vikingfiles':
...
async def album_upload(metadata, user):
    ...
    if user_mode in ['Gofile', 'Buzzheavier', 'Vikingfiles']:
        ...
        # Handle ZIP path (Prioritas)
        if metadata.get('zip_path'):
             target = metadata['zip_path']
             # Karena sekarang utils.py force single zip, ini pasti string. 
             # Tapi jika masih ada sisa list dari cache lama, ambil elemen pertama.
